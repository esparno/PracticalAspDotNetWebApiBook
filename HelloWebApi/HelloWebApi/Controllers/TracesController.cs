    ...
    public class TracesController : ApiController
    {
        public HttpResponseMessage Get()
        {
            ...
            if (entries != null && entries.Count >0)
            {
                ...
                {
                    if (!String.IsNullOrEmpty(entry.Operation) && !String.IsNullOrEmpty(entry.Operator) && !String.IsNullOrEmpty(entry.Category))
                    {
                        if (entry.Kind == TraceKind.Begin)
                        {
                            var end = entries.FirstOrDefault( e => entry.RequestId.Equals(e.RequestId) && entry.Operator.Equals(e.Operator) &&
                                entry.Operation.Equals(e.Operation) && entry.Category.Equals(e.Category) && e.Kind == TraceKind.End);
